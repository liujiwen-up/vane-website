...
Common Crawl 很适合搜索、检索和模型训练流程，但 WET 文件仍需经过解码、过滤，并被切分成适合模型处理的单元。本教程沿着 [`examples/common_crawl.py`](https://github.com/AstroVela/vane/blob/v0.1.0/examples/common_crawl.py) 的主线，从原始 WARC 结构记录一直走到页面、文本块与嵌入产物。
...
所有参数与 WET 解析实现请查看 [0.1.0 完整源码](https://github.com/AstroVela/vane/blob/v0.1.0/examples/common_crawl.py)。
