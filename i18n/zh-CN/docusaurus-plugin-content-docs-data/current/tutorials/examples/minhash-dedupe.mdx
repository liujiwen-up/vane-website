...
重复和近似重复文本会扭曲训练语料、检索索引和分析结果。本教程沿着 [`examples/minhash_dedupe.py`](https://github.com/AstroVela/vane/blob/v0.1.0/examples/minhash_dedupe.py) 完成一条依赖较少的完整去重流程：规范化文本、计算 MinHash 签名、用局部敏感哈希生成候选对、验证候选对，再为每个连通分量保留一个代表行。
...
CSV 与 HTML 加载、概率积分和结果序列化的完整实现请查看 [0.1.0 完整源码](https://github.com/AstroVela/vane/blob/v0.1.0/examples/minhash_dedupe.py)。
