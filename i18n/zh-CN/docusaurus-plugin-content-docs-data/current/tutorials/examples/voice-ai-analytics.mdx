...
音频被转换为带时间戳、类型明确的数据行后，会更容易搜索和分析。加载音频后，本教程沿着 [`examples/voice_ai_analytics.py`](https://github.com/AstroVela/vane/blob/v0.1.0/examples/voice_ai_analytics.py) 完成四个处理阶段：转写音频、总结每条转录文本、把片段展开为字幕行，再为片段文本生成检索嵌入。
...
样例 WAV 生成、OpenAI 摘要提示、模型加载错误信息以及全部参数请查看 [0.1.0 完整源码](https://github.com/AstroVela/vane/blob/v0.1.0/examples/voice_ai_analytics.py)。
