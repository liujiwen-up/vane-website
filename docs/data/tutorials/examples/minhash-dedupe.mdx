...
Duplicate and near-duplicate text can distort training corpora, retrieval indexes, and analytics. This tutorial follows [`examples/minhash_dedupe.py`](https://github.com/AstroVela/vane/blob/v0.1.0/examples/minhash_dedupe.py) through a complete, dependency-light deduplication pipeline: normalize text, compute MinHash signatures, use locality-sensitive hashing to generate candidate pairs, verify those pairs, and keep one representative per connected component.
...
See the [complete 0.1.0 source](https://github.com/AstroVela/vane/blob/v0.1.0/examples/minhash_dedupe.py) for CSV and HTML loading, probability integration, and output serialization.
